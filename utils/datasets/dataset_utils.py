...
# fungitastic-m has some images that are truncated/corrupted (few bytes not processed) and cannot be opened by PIL
# add the following lines to load truncated images without error
...
def load_dataset(dataset_root, split, preprocess, tokenized_text_prompts=None, pl_list=None):
...
def load_oracle_dataset(dataset_root, split, preprocess, tokenized_text_prompts=None, pl_list=None):
    
    dataset = MyOracleDataset(dataset_root=dataset_root,
                        split=split, transform=preprocess, 
                        tokenized_text_prompts=tokenized_text_prompts)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.tokenized_text_prompts is not None:
            ...
            tokenized_text = torch.zeros(1, 1).long()
...
class MyOracleDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return img, label, file_path, source
