...
def compute_clap_loss(logits, labels, classifier_head, base_text_features, alpha_constraint):

    loss_ce = F.cross_entropy(logits, labels)

    print(classifier_head.linear.weight.data.shape, base_text_features.shape, alpha_constraint.shape)
    print('alpha_constraint shape:', alpha_constraint.shape)

    # Compute l2 constraint
    disimilitude = (classifier_head.linear.weight.data - base_text_features).pow(2).sum(-1)
    loss_constraint = torch.mean(alpha_constraint * disimilitude)

    loss = loss_ce + loss_constraint

    print(f'loss_ce: {loss_ce.item()}, loss_constraint: {loss_constraint.item()}')

    return loss, loss_ce, loss_constraint


def extract_zeroshot_logits(args, train_loader, model, classifier_head):

    # get the logits and labels from the train_loader
    device = args.device
    logit_scale = args.logit_scale

    args.logger.info(f"Extracting zero-shot logits with few-shot data augmentation ......")
    assert args.pre_extracted is False, 'args.pre_extracted must be False.'
    ...
    return targets_list, logits_list


def init_lagrangian_multipliers(args, zeroshot_labels, zeroshot_logits):

    args.logger.info(f"Initializing lagrangian multipliers ......")
    device = args.device
    with torch.no_grad():
        # Get one-hot encoding ground-truth
        labels_one_hot = torch.nn.functional.one_hot(zeroshot_labels).cpu()

        # Get zero_shot performance
        performance = torch.diag(torch.softmax(zeroshot_logits, -1).t() @ labels_one_hot.to(torch.float32)) /\
                                labels_one_hot.sum(0)

    alpha_constraint = torch.clone(performance).to(device)
    # penalty_parameter = torch.zeros_like(alpha_constraint).to(device)

    args.logger.info(f"Lagrangian multipliers initialized: {list(torch.round(alpha_constraint.detach(), decimals=3).cpu().numpy())}")
    args.logger.info(f"avg Zero-shot performance: {np.mean(torch.round(performance.detach(), decimals=3).cpu().numpy())}")

    return alpha_constraint


def train_CLAP(args, logger, loss_logger, model, classifier_head, tokenized_text_prompts, preprocess, train_loader, val_loader, test_loader, reload_model=False):
    """ Train the model with CLAP"""
    ...
    logger.info(f"Start CLAP ......")

    model.eval()
    classifier_head.eval()

    # initialize the lagrangian multipliers
    zeroshot_labels, zeroshot_logits = extract_zeroshot_logits(args, train_loader, model, classifier_head)
    alpha_constraint = init_lagrangian_multipliers(args, zeroshot_labels, zeroshot_logits)

    # extract the features for CLAP probing
    new_train_fea_path = f'{args.dataset_root}/pre_extracted/{args.dataset}_{args.model_cfg}_{args.seed}_train_features_new.pth'
    new_val_fea_path = f'{args.dataset_root}/pre_extracted/{args.dataset}_{args.model_cfg}_{args.seed}_val_features_new.pth'
    new_test_fea_path = f'{args.dataset_root}/pre_extracted/{args.dataset}_{args.model_cfg}_{args.seed}_test_features_new.pth'
    ...
    val_loader = extract_dataloader(args, model, args.val_split, new_val_fea_path, preprocess, tokenized_text_prompts)
    test_loader = extract_dataloader(args, model, args.test_split, new_test_fea_path, preprocess, tokenized_text_prompts)
    logger.info(f'Extracted train, val, test dataloader for CLAP probing.')
    ...
    base_text_features = args.zeroshot_weights.clone().to(args.device)

    # switch classifier head to train mode
    classifier_head.train()
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for inputs, labels, text, source in train_loader:
            ...
            # total_loss = loss(logits, labels)            
            total_loss, loss_ce, loss_constraint = compute_clap_loss(logits, labels, classifier_head, base_text_features, alpha_constraint)
        ...
        train_loss_avg = train_loss_sum / len(train_loader)
        loss_ce_avg = loss_ce_sum / len(train_loader)
        loss_constraint_avg = loss_constraint_sum / len(train_loader)
        ...
        logger.info(f"Epoch {int(epoch)}, Iter {num_iter}, lr_cls: {round(lr_classifier, 8)}, "
                    f"Trn Loss: {round(train_loss_avg, 6)}, "
                    f"Loss_CE: {round(loss_ce_avg, 6)}, Loss_Constraint: {round(loss_constraint_avg, 6)}, "
                    f"Val Loss: {round(val_loss, 6)}, Val Acc: {round(val_acc, 3)}, "
                    f"Test Acc: {round(test_acc, 3)}")
...
def train_ce(args, logger, loss_logger, model, classifier_head, train_loader, val_loader, test_loader, reload_model=False):
    ...
    is_encoder = (cfgs[1] == 'openclip' or cfgs[1] == 'clip' or cfgs[1] == 'bioclip')
