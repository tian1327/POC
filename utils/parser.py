import argparse
import os
from utils.extras import get_class_num_list, str2bool, METHOD_WITH_FROZEN_BACKBONES
import yaml
...
def parse_args():
    ...
    parser.add_argument('--model_cfg', type=str, default='vitb32_openclip_laion400m',
                        choices=['vitb32_openclip_laion400m',
    ...
    parser.add_argument('--data_source', type=str, default='fewshot',
                        choices=['fewshot', 'retrieved', 'fewshot+retrieved', 'dataset-cls',
                                 ...
                                 'ltrain+val+retrieved', 'ltrain+val+unlabeled+retrieved',
                                 'fewshot+unlabeled', 'fewshot+retrieved+unlabeled'],
                        help='training data source.')
    parser.add_argument('--unlabeled_data_source', type=str, default='fewshot+unlabeled',
                        choices=['unlabeled', 'fewshot+unlabeled',
                                 'fewshot+unlabeled+retrieved',],
                        help='training data source.')
    ...
    parser.add_argument('--oracle_split', type=str, default='u_train_in_oracle.txt', help='oracle data file name.')
    ...
    parser.add_argument('--scale_text_embedding', action='store_true', default=False, help='Scale the text embedding by logit scale.')
    ...
    parser.add_argument('--skip_stage1', action='store_true', default=False, help='Set to skip stage 1 finetuning')
    parser.add_argument('--skip_stage2', action='store_true', default=False, help='Set to skip stage 2 probing')
    ...
    # MoCo
    parser.add_argument('--moco_lr', type=float, default=1.5e-4, help='Learning rate for moco.')
    parser.add_argument('--moco_wd', type=float, default=0.1, help='weight decay for moco.')
    parser.add_argument('--moco_bsz', type=int, default=256, help='Batch Size for MoCo')
    parser.add_argument('--moco_m', type=int, default=0.999, help='momentum for MoCo')
    parser.add_argument('--moco_k', type=int, default=2048, help='queue size for MoCo')
    parser.add_argument('--moco_loss_weight', type=float, default=0.01, help='MoCo loss weight for joint training')

    # fixmatch
    parser.add_argument('--check_logits', action='store_true', help='check the logits distribution.')
    parser.add_argument('--temp_scheme', type=str, default='fewshot+retrieved+unlabeled',
                        choices=['none', 'fewshot', 'retrieved', 'fewshot+retrieved',
                                 'fewshot+unlabeled', 'fewshot+retrieved+unlabeled'],
                        help='training data source.')
    parser.add_argument('--use_ema', action='store_true', help='use EMA models.')
    parser.add_argument('--ema_decay', type=float, default=0.999, help='EMA decay rate.')
    parser.add_argument('--mu', type=int, default=1, help='number of times of the batch size for few-shot data.')
    parser.add_argument('--threshold', type=float, default=0.85, help='confidence threshold to retain the pseudo-labels.')
    parser.add_argument('--lambda_u', type=float, default=1.0, help='weight for the consistency loss.')
    parser.add_argument('--lambda_contrastive', type=float, default=1.0, help='weight for the contrastive loss.')
    parser.add_argument('--T', default=1.0, type=float, help='pseudo label temperature')
    ...
    parser.add_argument('--iterations', type=int, default=500, help='number of iterations per epoch')
    ...
    parser.add_argument('--lr_temp', type=float, default=1e-4, help='Learning rate for the loss temperature.')
    ...
    # loss
    parser.add_argument('--loss_name', type=str, default='CE', choices=['CE', 'WeightedCE', 'Focal', 'BalancedSoftmax',
                                                                        'LCA'], help='type of loss function to use.')
    ...
    if args.method in METHOD_WITH_FROZEN_BACKBONES:
    ...
    # for CLAP, we set pre_extracted to False to enable data augmentation in few-shot data
    if args.method == 'CLAP':
        args.pre_extracted = False
        args.freeze_visual = True
    ...
    args.oracle_split = [[args.oracle_split], [os.path.join(args.dataset_path, args.dataset)]]
    ...
    if args.data_source == 'fewshot':
        ...
        if args.method == 'fixmatch' or args.method == 'fixmatch-joint' \
            ...
            # args.train_split = [[f'ltrain.txt'], # test DebiasPL effectiveness with LT training data
            #                 [os.path.join(args.dataset_path, args.dataset)]]

            # unlabeled data
            if args.unlabeled_data_source == 'fewshot+unlabeled': # common practice in FixMatch
                args.u_train_split = [[f'fewshot{args.shots}_seed{args.seed}.txt',
                                    args.unlabeled_split],
                                [os.path.join(args.dataset_path, args.dataset),
                                os.path.join(args.dataset_path, args.dataset)]]

            elif args.unlabeled_data_source == 'fewshot+unlabeled+retrieved': # add retrieved data to unlabeled set
                args.u_train_split = [[f'fewshot{args.shots}_seed{args.seed}.txt',
                ...
                raise NotImplementedError('unlabeled_data_source not supported')

        else:
            # using oracle data for unlabeled_split
            args.train_split = [[f'fewshot{args.shots}_seed{args.seed}.txt',
                                 args.unlabeled_split],
                                [os.path.join(args.dataset_path, args.dataset),
                                os.path.join(args.dataset_path, args.dataset)]]
        ...
        if args.method == 'fixmatch' \
            or args.method == 'fixmatch-joint' \
            or args.method == 'fixmatch-retraincls' \
            or args.method == 'debiasPL' \
            or args.method == 'debiasPL-joint' \
            ...
            # oracle data for unlabeled_split
            args.train_split = [[f'fewshot{args.shots}_seed{args.seed}.txt',
                                args.retrieval_split,
                                args.unlabeled_split],
                            [os.path.join(args.dataset_path, args.dataset),
    ...
    # elif args.data_source == 'ltrain+val+retrieved':
    #     args.train_split = [[f'ltrain+val.txt', args.retrieval_split],
    #                         [os.path.join(args.dataset_path, args.dataset),
    #                          os.path.join(args.retrieved_path, args.dataset)]]
    #     args.val_split = [[f'test.txt'], [os.path.join(args.dataset_path, args.dataset)]] # use test set as val set
    #     args.early_stop = True
