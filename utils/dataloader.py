from .datasets.dataset_utils import load_dataset, TensorDataset, MyUnlabeledDataset, TextTensorDataset, load_oracle_dataset
...
import numpy as np
from .extras import TransformFixMatch, MoCoTransform


def extract_dataloader(args, best_model, split, fea_path, preprocess, tokenized_text_prompts=None, bsz=128):
...
def get_dataloader(args, train_split, val_split, test_split, tokenized_text_prompts, preprocess, utrain_labels=None):
    ...
    train_dataset = load_dataset(dataset_root=args.dataset_root,
                                split=train_split,
                                preprocess=transform(224, 'train'), # note that here train preprocess is used
                                tokenized_text_prompts=tokenized_text_prompts,
                                )
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.bsz, pin_memory=True,
                            shuffle=True, 
                            # drop_last=True, # need to comment out this for mollusca dataset 4-shot
                            drop_last=False,
                            num_workers=args.num_workers)

    val_loader = DataLoader(val_dataset, batch_size=256, drop_last=False, pin_memory=True,
...
def get_retrieve_fewshot_dataloader(args, retrieve_data, tokenized_text_prompts=None, preprocess=None, bsz=32, utrain_labels=None):
    ...
    train_dataloader_retr = DataLoader(train_dataset_retr, batch_size=bsz, pin_memory=True,
                            shuffle=True, drop_last=True, num_workers=args.num_workers)

    return train_dataloader_retr


def get_unlabeled_dataloader(args, unlabeled_split, is_openclip=True):

    u_train_dataset = MyUnlabeledDataset(dataset_root=args.dataset_root,
                                        split=unlabeled_split,
                                        transform=TransformFixMatch(224, is_openclip),
                                        )

    u_train_dataloader = DataLoader(u_train_dataset, batch_size=args.bsz*args.mu, pin_memory=True,
                            shuffle=True, drop_last=True,
                            num_workers=args.num_workers,
                            # num_workers=16 # use more workers to speed up fixmatch
                            )

    return u_train_dataloader

def get_moco_dataloader(args, unlabeled_split, is_openclip=True):

    u_train_dataset = MyUnlabeledDataset(dataset_root=args.dataset_root,
                                        split=unlabeled_split,
                                        transform=MoCoTransform(224, is_openclip),
                                        )

    u_train_dataloader = DataLoader(u_train_dataset, batch_size=args.moco_bsz, pin_memory=True,
                            shuffle=True, drop_last=True, num_workers=args.num_workers)
...
def set_dataloaders(args, model, tokenized_text_prompts, preprocess, logger):
    ...
    train_fea_path, val_fea_path, test_fea_path = None, None, None
    ...
    elif args.method == 'moco-cutmix':
        u_train_dataloader = get_moco_dataloader(args, args.u_train_split)
        train_loader = (train_loader, u_train_dataloader)

    elif args.method == 'MoCo': # unsupervised training on unlabeled data
        u_train_dataloader = get_moco_dataloader(args, args.u_train_split)
        logger.info(f'len(u_train_dataloader): {len(u_train_dataloader)}')
        train_loader = u_train_dataloader


    return train_loader, val_loader, test_loader, train_fea_path, val_fea_path, test_fea_path
...
def get_oracle_data_loader(args, preprocess, tokenized_text_prompts):

    oracle_dataset = load_oracle_dataset(dataset_root=args.dataset_root,
                                # split=args.oracle_split,
                                split=args.test_split, # use the test split for getting pseudo-labels
                                preprocess=preprocess,
                                tokenized_text_prompts=tokenized_text_prompts,
                                )

    oracle_loader = DataLoader(oracle_dataset, batch_size=1024, drop_last=False, pin_memory=True,
                            shuffle=False, num_workers=args.num_workers)
