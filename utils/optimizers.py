...
from utils.extras import METHOD_WITH_FROZEN_BACKBONES
...
def set_optimizer(args, params, train_loader, moco_wd=None):
    ...
    args.logger.info(f"Warmup iters: {warmup_iter}, Warmup lr: {warmup_lr}")

    if args.method == "CLAP":
        warmup_lr = 1e-5 # for CLAP
        args.logger.info(f"Overwritting warmup lr to {warmup_lr} for CLAP")
        scheduler = get_warmup_scheduler(optimizer=optimizer, scheduler=base_scheduler, warmup_iter=warmup_iter, warmup_lr=warmup_lr, warmup_type="constant")
    else:
        scheduler = get_warmup_scheduler(optimizer=optimizer, scheduler=base_scheduler, warmup_iter=warmup_iter, warmup_lr=warmup_lr)
...
def set_params(args, model, classifier_head, logger, dataset_classifier_head=None):
    ...
    elif args.method in METHOD_WITH_FROZEN_BACKBONES:
...
def get_warmup_scheduler(optimizer, scheduler, warmup_iter = 50, warmup_lr = 1e-6, warmup_type = "linear"):
