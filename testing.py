...
def load_model(args, logger, model, test_loader=None, classifier_head=None, is_encoder=True):
    ...
    is_encoder = (args.model_cfg.split('_')[1] == 'openclip' or args.model_cfg.split('_')[1] == 'clip' or args.model_cfg.split('_')[1] == 'bioclip')
...
def validate(args, data_loader, model, logger, loss, logit_scale, classifier_head = None,
             show_confusion_matrix = False, device='cuda',
             dataset='semi-aves', output_dir='output',
             predict_labels=False, predict_split='u_train',
             is_encoder=True):
    ...
    if predict_labels:
        ...
        with open(predicted_label_file, 'w') as f:
            for item, logit in zip(predicted_labels, max_logits):
                f.write("%s %s\n" % (item, logit))
        args.logger.info(f'Predicted labels saved to: {predicted_label_file}')
...
def extract_confidence(args, model, classifier_head, test_loader):
    ...
    with torch.no_grad():
        ...
        for i, val_data in enumerate(test_loader):
            inputs, labels, path, source = val_data
            inputs = inputs.to(device)
            ...
            pseudo_label = torch.softmax(logit.detach()/args.T, dim=-1)
            max_probs, targets_u = torch.max(pseudo_label, dim=-1)
            # mask = max_probs.ge(args.threshold).float()

            max_probs_list.append(max_probs.cpu().numpy())

    confidence = np.concatenate(max_probs_list)
    args.logger.info(f'confidence.shape: {confidence.shape}')
    confidence_list = confidence.tolist()

    # save the predicted labels to a csv file
    out_file = f'plots_and_tables/plot_confidence/{args.model_cfg}_{args.T}_test_confidence.csv'
    df = pd.DataFrame(confidence_list)
    # add header
    df.columns = ['max_probs']
...
def extract_topk_predictions(args, model, classifier_head, data_loader,
    ...
    with torch.no_grad():
        ...
        for i, val_data in enumerate(data_loader):
            ...
            K = 20
            topk_logits, topk_indices = torch.topk(logit, k=min(K, logit.size(1)), dim=1)
    ...
    if not os.path.exists(f'output/predictions_topk_{args.method}/'):
        os.makedirs(f'output/predictions_topk_{args.method}/')

    predicted_label_file = f'output/predictions_topk_{args.method}/{args.method}_{args.model_cfg}_{args.dataset}_{args.shots}_{args.seed}_topk_test_predictions.json'
...
def extract_pseudolabels(args, model, classifier_head, data_loader, logger,
                     show_confusion_matrix=True, is_encoder=True,
                     device='cuda', predict_labels=True):
    ...
    if predict_labels:
        ...
        # save the predicted labels to a text file
        # predicted_label_file = f'{args.output_dir}/{args.dataset}_predicted_labels.txt'
        predicted_label_file = f'{args.output_dir}/{args.dataset}_test_predicted_labels.txt'

        with open(predicted_label_file, 'w') as f:
            for path, truth, pred, confidence in zip(file_paths, ground_truth_labels, predicted_labels, max_logits):
                f.write(f"{path} {truth} {pred} {confidence}\n")
        args.logger.info(f'Predicted labels saved to: {predicted_label_file}')
