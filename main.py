...
# import cv2
from utils.training import set_training_seed, train_probing, run_zeroshot, train_CMLP, train_CLAP, \
    ...
    train_saliencymix2, train_attentivemix2, train_CMO, train_supervised_contrastive, \
    train_balanced_contrastive, train_fixmatch_retraincls, train_fixmatch2, train_fixmatch3, \
    train_debiasPL, train_debiasPL_cutmix,\
    train_moco_cutmix, train_moco, train_fixmatch_supervised_contrastive

from utils.dataloader import extract_train_dataloader, extract_dataloader, set_dataloaders, set_text_dataloader, get_oracle_data_loader
...
def run_stage1_finetuning(args, logger, model, preprocess, tokenized_text_prompts):
    ...
    train_loader, val_loader, test_loader, \
        train_fea_path, val_fea_path, test_fea_path = set_dataloaders(args, model, tokenized_text_prompts, preprocess, logger)
    ...
    args.params = params # record the params for MoCo training update
    ...
    # get the pseudo labels for the unlabeled data using the loaded model
    if args.get_pseudolabels:
        load_model(args, logger, model, test_loader, classifier_head, True)
        oracle_loader = get_oracle_data_loader(args, preprocess, tokenized_text_prompts)
        unlabeled_acc, unlabeled_confusion_matrix = extract_pseudolabels(args, model, classifier_head, oracle_loader, logger)
        logger.info(f"+++++ Unlabeled Acc: {round(unlabeled_acc, 3)}")
        exit()
    ...
    if args.method == 'probing':
        ...
        best_model, best_head, best_records, \
            best_logit_scale, val_loader, test_loader = train_CLAP(args, logger, loss_logger, model, classifier_head, \
                                                                      tokenized_text_prompts, preprocess, \
                                                                      train_loader, val_loader, test_loader, reload_model)
        ...
        #---------- add contrastive loss to FixMatch loss
        # best_model, best_head, \
        #     best_records, best_logit_scale = train_fixmatch_contrastive(args, logger, loss_logger, model, classifier_head, \
        #                                                     train_loader, val_loader, test_loader, reload_model)

        #---------- add Supervised contrastive loss to FixMatch loss
        # best_model, best_head, \
        #     best_records, best_logit_scale = train_fixmatch_supervised_contrastive(args, logger, loss_logger, model, classifier_head, \
        #                                                     train_loader, val_loader, test_loader, reload_model)


        # best_model, best_head, \
        #     best_records, best_logit_scale = train_fixmatch2(args, logger, loss_logger, model, classifier_head, \
        #                                                     train_loader, val_loader, test_loader, reload_model)

        # best_model, best_head, \
        #     best_records, best_logit_scale = train_fixmatch3(args, logger, loss_logger, model, classifier_head, \
        #                                                     train_loader, val_loader, test_loader, reload_model)

    elif args.method == 'fixmatch-joint':
        best_model, best_head, \
            best_records, best_logit_scale = train_fixmatch_cutmix(args, logger, loss_logger, model, classifier_head, \
                                                            train_loader, val_loader, test_loader, reload_model)


    elif args.method == 'fixmatch-retraincls': # bs is labeled, bs*mu is unlabeled
        best_model, best_head, \
            best_records, best_logit_scale = train_fixmatch_retraincls(args, logger, loss_logger, model, classifier_head, \
                                                            train_loader, val_loader, test_loader, reload_model)
    ...
    elif args.method == 'debiasPL-joint':
        best_model, best_head, \
            best_records, best_logit_scale = train_debiasPL_cutmix(args, logger, loss_logger, model, classifier_head, \
                                                            train_loader, val_loader, test_loader, reload_model)
    ...
    elif args.method == 'moco-cutmix': # MoCo + cutmix
        best_model, best_head, \
            best_records, best_logit_scale = train_moco_cutmix(args, logger, loss_logger, model, classifier_head, \
                                                          train_loader, val_loader, test_loader, reload_model)

    elif args.method == 'MoCo': # MoCo
        best_model, best_head, \
            best_records, best_logit_scale = train_moco(args, logger, loss_logger, model, classifier_head, \
                                                          train_loader, val_loader, test_loader, reload_model)
    ...
    # remove the extracted features if exists
    for fea_path in [train_fea_path, val_fea_path, test_fea_path]:
        if fea_path and os.path.exists(fea_path):
            os.remove(fea_path)
            logger.info(f"Removed extracted features: {fea_path}")
        else:
            pass
...
def run_stage2_probing(stage1_best_model_path, test_loader):
    ...
    test_scores = calculate_scores(test_confusion_matrix)
    logger.info(f"+++++ Stage 2 linear probing Test Acc: {round(test_acc, 3)}")
    ...
    logger.info(f'stage 2 LP Best Model saved to: {best_model_path}')
...
def run_stage2_FSFT(stage1_best_model_path, test_loader):
...
if __name__ == '__main__':
    ...
    args.model_logit_scale = model.logit_scale if hasattr(model, 'logit_scale') else None
    # logger.info(f"model.logit_scale: {model.logit_scale}")
    # stop
    ...
    args.zeroshot_weights = zeroshot_head.linear.weight.data
    classifier_head.to(args.device)

    # run finetuning for stage 1
    stage1_acc, stage1_best_model_path, test_loader, results = run_stage1_finetuning(args, logger, model, preprocess, tokenized_text_prompts)
    ...
    if not args.skip_stage2:
        stage2_lp_acc, stage2_best_model_path = run_stage2_probing(stage1_best_model_path, test_loader)
    else:
    ...
    if not args.skip_stage2:
        stage2_fsft_acc, stage2_best_model_path = run_stage2_FSFT(stage1_best_model_path, test_loader_copy)
    else:
