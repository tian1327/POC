...
def keep_answer_only(text: str, strip_all_tags: bool = False) -> str:
    ...
    if m:
        ...
        # No explicit <answer> — remove thoughts and return what's left
        out = THINK_RE.sub("", text).strip()
    if strip_all_tags:
        out = TAGS_RE.sub("", out).strip()
...
def print_prompt_for_debug(processor: AutoProcessor, messages: List[Dict[str, Any]]):
    # For human-readable printing, exclude assistant prefill
...
def prompt_text_for_csv(processor: AutoProcessor, messages: List[Dict[str, Any]]) -> str:
    # Store the user-facing prompt in CSV
    text = processor.apply_chat_template(messages, tokenize=False, add_generation_prompt=False)
    text = _strip_chat_wrappers(text)
...
def main():
    ...
    ap.add_argument("--hf-model-name-or-path", default="THUDM/GLM-4.1V-9B-Thinking")
    ...
    try:
        ...
        for i, rel in enumerate(iterable, 1):
            ...
            try:
                ...
                answer = keep_answer_only(text, strip_all_tags=True)
