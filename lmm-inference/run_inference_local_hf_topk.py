...
TEMPLATE_MAP = {
    ...
    "top3-multimodal-16shot-with-confidence_ranking": ("top3_multimodal_ranking.txt", "top5_desc_multimodal_rank", dict(with_conf=True, with_sci=True)),
    "top7-multimodal-16shot-with-confidence_ranking": ("top7_multimodal_ranking.txt", "top5_desc_multimodal_rank", dict(with_conf=True, with_sci=True)),
    "top10-multimodal-16shot-with-confidence_ranking": ("top10_multimodal_ranking.txt", "top5_desc_multimodal_rank", dict(with_conf=True, with_sci=True)),
    "top15-multimodal-16shot-with-confidence_ranking": ("top15_multimodal_ranking.txt", "top5_desc_multimodal_rank", dict(with_conf=True, with_sci=True)),
    "top20-multimodal-16shot-with-confidence_ranking": ("top20_multimodal_ranking.txt", "top5_desc_multimodal_rank", dict(with_conf=True, with_sci=True)),
...
def build_topk_index(topk_json_path: Path, image_dir: Path, probe_n: int = 10) -> Dict[str, Dict[str, Any]]:    
    ...
    keys = list(data.keys())
    probe_keys = keys[:probe_n] if keys else []
    if data and all(k.isdigit() for k in probe_keys):
        for k in probe_keys:
            v = data[k]
...
def render_top5_multimodal(template_txt: str, with_conf: bool, with_sci: bool,
                           ...
                           debug_resize: bool = False) -> List[Dict[str, Any]]:
    id2c, id2s = tax["id2common"], tax["id2sci"]
...
def render_top5_desc_multimodal(
    ...
    if wants_list_placeholder:
        ...
        lines = []
        for i, (cid, p) in enumerate(candidates, 1):
            lines.append(_header_line(i, cid, p))
        species_list_text = "\n".join(lines)
...
def render_top5_contrastive_group_text(
    ...
    q_abs = str(test_img)
    # also try a relative key like "test/28.jpg"
    rel_key = q_abs
...
def main():
    ...
    ap.add_argument("--topk", type=int, default=5, help="Number of candidates (k) to include from topk_json for top-k prompts.")
    ...
    if needs_top5:
        ...
        probe_n = max(10, int(args.topk or 5))
        topk_index = build_topk_index(Path(args.topk_json), image_dir, probe_n=probe_n)
    ...
    def build_messages_for(rel_img_path: str) -> List[Dict[str, Any]]:
        ...
        for k in range(min(args.topk, len(cls))):
            try:
                cid = int(cls[k])
            except Exception:
                continue
        ...
        if mode == "top5_multimodal":
            ...
            return render_top5_multimodal(template_txt, with_conf, with_sci, q_abs, pairs, tax,
                                          ...
                                          debug_resize=args.debug_resize)
            ...
            if needs_top5:
                ...
                if ("multimodal" in mode) and args.ref_image_dir:
                    ...
                    if rec:
                        ...
                        for k in range(min(args.topk, len(rec.get("topk_cls") or []))):
                            try:
