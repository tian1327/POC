...
import os, sys, json, argparse, time, csv, re, unicodedata, io
...
def _response_format_for(prompt_key: str) -> str:
    WITH_EXPLANATION = {"zeroshot-explanation", "zeroshot-all200-explanation"}
...
def messages_to_prompt_text(messages: List[Dict[str, Any]]) -> str:
    """Flatten our single-user message into text with [IMAGE] placeholders."""
    if not messages:
        return ""
...
def print_prompt_for_debug(messages: List[Dict[str, Any]]):
    print("----- PROMPT -----")
    print(messages_to_prompt_text(messages))
    print("------------------")



def _pil_to_bytes_jpeg(pil: Image.Image, quality: int = 92) -> bytes:
    buf = io.BytesIO()
    pil.convert("RGB").save(buf, format="JPEG", quality=quality)
    return buf.getvalue()

def _to_openai_content_parts(messages: List[Dict[str, Any]], client: OpenAI) -> List[Dict[str, Any]]:
    parts: List[Dict[str, Any]] = []
        ...
        if p.get("type") == "text":
            ...
            if isinstance(img, (str, Path)):
                with Image.open(img) as pil:
                    data = _pil_to_bytes_jpeg(pil)
            elif isinstance(img, Image.Image):
                data = _pil_to_bytes_jpeg(img)
            else:
                continue
...
def _price_for(model: str) -> Tuple[float, float]:
    row = PRICE_TABLE_PER_MTOK.get(model, {})
    return float(row.get("in", 0.0)), float(row.get("out", 0.0))



def main():
    ap = argparse.ArgumentParser(description="Template-driven GPT Vision Runner")
    ...
    ap.add_argument("--hf-model-name-or-path", default="gpt-5-mini")
    ap.add_argument("--api-model", default="ignored")
    ap.add_argument("--api-base", default="ignored")
    ap.add_argument("--ollama-model", default="ignored")
    ...
    ap.add_argument(
        "--resize-candidates", dest="resize_candidates", type=int, default=1200,
    ...
    model_name = (args.hf_model_name_or_path or "").strip()
    if not model_name.startswith("gpt-"):
        print(f"[warn] --hf-model-name-or-path '{model_name}' does not look like a GPT model id. Using it anyway.")
    ...
    def build_messages_for(rel_img_path: str) -> List[Dict[str, Any]]:
        ...
        if mode == "top5_multimodal":
            ...
            return render_top5_multimodal(template_txt, with_conf, with_sci, q_abs, pairs, tax,
                                          ...
                                          debug_resize=False, with_taxonomy=bool(flags.get("with_taxonomy")),)
    ...
    if args.dry_run:
        ...
        print_prompt_for_debug(build_messages_for(df.iloc[0]["image_path"]))
    ...
    if OpenAI is None:
        sys.exit("openai python package not found. Install: pip install openai python-dotenv")
    load_dotenv()  
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        sys.exit("OPENAI_API_KEY not set (in environment or .env).")
    ...
    total_out_tokens = 0
    total_cost_usd   = 0.0
    price_in_per_m, price_out_per_m = _price_for(model_name)

    try:
        if write_header:
            out_writer.writerow(out_cols)

        print(">> Starting generation (OpenAI GPT) ...")
        ...
        for i, rel in enumerate(iterable, 1):
            ...
            try:
                ...
                prompt_for_csv = messages_to_prompt_text(messages)

                content_parts = _to_openai_content_parts(messages, client)
                # Use a per-request timeout
                resp = client.with_options(timeout=REQ_TIMEOUT_S).responses.create(
                    model=model_name,
                ...
                try:
                    ...
                    if getattr(resp, "output", None):
                        first = resp.output[0]
                        for piece in getattr(first, "content", []) or []:
                            if getattr(piece, "type", "") in ("output_text", "text"):
                                answer = getattr(piece, "text", "") or ""
                                break
                ...
                inc_cost = (in_tok/1_000_000.0)*price_in_per_m + (out_tok/1_000_000.0)*price_out_per_m
                ...
                iterable.set_postfix_str(f"in:{in_tok} out:{out_tok}")
        ...
        print(f"RUN USAGE: input_tokens={total_in_tokens} output_tokens={total_out_tokens} "
              f"cost=${total_cost_usd:.4f} (using internal price table)")
